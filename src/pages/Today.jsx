...
export default function Today({ openModal }) {
  ...
  // On load: scroll so current time is visible ~120px from top of scroll area
  useEffect(() => {
    if (loading || !scrollRef.current) return
    const target = nowTop != null ? Math.max(0, nowTop - 120) : 0
  ...
  // Tap on empty timeline area → pre-fill time in modal
  function handleGridTap(e) {
    if (e.target !== e.currentTarget) return
    const rect  = e.currentTarget.getBoundingClientRect()
    const y     = e.clientY - rect.top + (scrollRef.current?.scrollTop ?? 0)
    const start = snapToTime(y)
    openModal({ defaultDate: todayStr, defaultStartTime: start })
  }
  ...
  return (
    <div className="flex flex-col bg-warm-100 overflow-hidden" style={{ height: '100%' }}>
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto scrollbar-none px-4 pb-28"
      >
        <div className="flex">
          {/* Time labels */}
          ...
          >
            ...
            {HOURS.map(h => (
              <div
                key={`half-${h}`}
                className="absolute left-0 right-0 border-t border-warm-200/50 border-dashed"
